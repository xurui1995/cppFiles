...
int main(int argc, char **argv) {
    ...
    std::cout << "打印expected_result" << expected_result << std::endl;   

    float *actual_output_ptr = tm.get(relu1_dst_desc.get_id()).get_data_handle<float>();


    std::cout << "打印*actual_output_ptr" << *actual_output_ptr << std::endl;   
    
    auto output_dims = relu1_dst_desc.get_dims();
    
    std::cout << "打印output_dims" << std::endl;
    for(auto dim : output_dims) {
        std::cout << dim << std::endl;
    }
    std::cout << "打印output_dims 结束" << std::endl;

    auto num_elem = product(output_dims);

    std::cout << "打印num_elem" << num_elem <<std::endl;
